...
async function sendCallback(
  ...
  const idempotencyKey = `${communicationId}:${eventType}:${uuidv4()}`;
    ...
    try {
      ...
      if (attempt === retries) {
        console.error(`[ChannelService] Callback failed after ${retries} attempts`, {
          communicationId, eventType, error: (err as Error).message,
        });
        return;
      }
...
export async function simulateDelivery(req: SendRequest): Promise<void> {
  ...
  if (!isOpened) return; // Delivered but not opened
  ...
  await sendCallback(callbackUrl, communicationId, campaignId, 'purchased', {
    ...
    order_id: `SIM-ORD-${uuidv4().slice(0, 8).toUpperCase()}`,
  });

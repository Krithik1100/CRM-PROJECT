...
`;
function extractDays(text, fallback = 60) {
    const lower = text.toLowerCase();
    const dayMatch = lower.match(/(\d+)\s*days?/);
    if (dayMatch)
        return Number(dayMatch[1]);
    if (lower.includes('3 months'))
        return 90;
    return fallback;
}
function inactiveSegment(days) {
    return {
        filterJson: {
            operator: 'AND',
            conditions: [
...
function fallbackSegmentQuery(query) {
...
function fallbackCampaignRecommendation(goal, customerStats) {
...
function enforceChannelDecision(goal, recommendation) {
    const lower = goal.toLowerCase();
    if (lower.includes('newsletter')) {
        return {
            ...recommendation,
...
exports.aiService = {
    async parseSegmentQuery(naturalLanguageQuery) {
...
Return ONLY valid JSON (no markdown, no explanation) with this exact structure:
{
  ...
  "sqlPreview": "a readable SQL WHERE clause preview (use plain English for dates like 'NOW() - INTERVAL 90 days')"
        ...
        try {
            ...
            logger_1.logger.error('AI segment parse failed, using deterministic fallback', { error, query: naturalLanguageQuery });
            return fallbackSegmentQuery(naturalLanguageQuery);
        }
    },
    async generateCampaignRecommendation(goal, customerStats) {
        const model = genAI.getGenerativeModel({ model: modelName });
        const prompt = `${DB_SCHEMA_CONTEXT}
...
Return ONLY valid JSON (no markdown) with this structure:
{
  ...
  "reasoning": "overall strategic reasoning for these recommendations (2-3 sentences)"
        ...
        try {
            ...
            const text = result.response.text().trim();
            const cleaned = text.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim();
            return enforceChannelDecision(goal, JSON.parse(cleaned));
        }
        catch (error) {
            logger_1.logger.error('AI campaign copilot failed, using deterministic fallback', { error, goal });

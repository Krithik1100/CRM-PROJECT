...
async function seedDatabase(customerCount = 500) {
    logger_1.logger.info(`Seeding database with ${customerCount} customers...`);
    const now = new Date();
    ...
    for (let i = 0; i < customerCount; i++) {
        ...
        const customer = await customer_repository_1.customerRepository.insertCustomer({
            external_id: `EXT-${(0, uuid_1.v4)().slice(0, 8).toUpperCase()}`,
            name,
        ...
        for (let j = 0; j < orderCount; j++) {
            ...
            await customer_repository_1.customerRepository.insertOrder({
    ...
    logger_1.logger.info(`✅ Seeded ${customerCount} customers with realistic order history`);
}
